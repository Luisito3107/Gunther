...
class Resolver {
    ...
    async getPlaylist(id) {
        ...
        try {
            ...
            return this.buildResponse("PLAYLIST_LOADED", this.autoResolve ? (await Promise.all(unresolvedPlaylistTracks.map(x => x.resolve()))).filter(Boolean) : unresolvedPlaylistTracks, {name: spotifyPlaylist.name, thumbnail: (spotifyPlaylist.images.length ? spotifyPlaylist.images[0].url : null)});
        }
        catch (e) {
    ...
    buildResponse(loadType, tracks = [], playlistInfo, exceptionMsg) {
        return Object.assign({
            loadType,
            tracks,
            playlistInfo: playlistInfo ? { name: playlistInfo.name, thumbnail: playlistInfo.thumbnail } : {}
