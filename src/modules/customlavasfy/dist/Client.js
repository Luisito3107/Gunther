...
class LavasfyClient {
    ...
    async otherApiRequest(endpoint, query) {
        await this.requestToken();
        let querystring = (query ? "?" : "");
        if (query) for (const key in query) { querystring += `${key}=${query[key].toString().replaceAll("#", "%23").replaceAll("&", "%26").replaceAll("?", "")}&`; }
        const node_superfetch_1 = __importDefault(require("node-superfetch"));
        var _a, _b, _c;
        try {
            if (!this.token)
            ...
            const { body: result } = await node_superfetch_1.default
                .get(`${`${this.baseURL}/${endpoint}`.replaceAll("//", "/")}${querystring}`)
